...
async def cb_guess_result(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    deadline = contest["deadline"].replace(tzinfo=timezone.utc) if contest.get("deadline") else now
    if now > deadline:
        await q.answer("⏰  Deadline has passed.", show_alert=True); return
    labels = {"win1": "1️⃣  Win 1", "draw": "🤝  Draw", "win2": "2️⃣  Win 2"}
    existing = await get_user_guess(cid, uid)
    if existing:
        prev = labels.get(existing["guess"], existing["guess"])
        await q.answer(f"✅  Already picked: {prev}", show_alert=True); return
    ...
    try:
        await q.edit_message_text(confirm_text, parse_mode=ParseMode.HTML)
    except Exception:
        try:
...
async def handle_admin_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin shortcut: MAIN or MENU → show main menu ──────────────────────────
    normalized = (update.message.text or "").strip().upper().replace("-", "").replace(" ", "")
    if normalized in ("MAIN", "MENU"):
