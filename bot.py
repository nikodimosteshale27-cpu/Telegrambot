...
MSG = {
    "en": {
        ...
        "age_text": (
            ...
            "  •  You must be <b>18 years or older</b>\n"
            "  •  Games involve real-money risk\n"
            "  •  Play responsibly\n\n"
            f"{D}\nConfirm your age to continue:"
        ),
        "age_continue": "✅  I am 18+  —  Continue",
        "age_exit":     "❌  Exit",
        "tos_text": (
            f"{LOGO}\n{D}\n\n"
            "📋  <b>Terms &amp; Responsible Use Policy</b>\n\n"
            "Please read our Terms of Service carefully.\n"
            "Tapping <b>Confirm</b> means you agree to all terms.\n\n"
            f"{D}"
        ),
        "tos_read":      "📄  Read Terms of Service",
        "tos_continue":  "✅  Confirm",
        "tos_exit":      "❌  Exit",
        "exit_message":  "👋  You've exited. Send /start anytime to begin again.",
        "maintenance":   f"{LOGO}\n{D}\n\n🔧  <b>Under Maintenance</b>\n\nThe bot is temporarily offline. Check back shortly!\n\n{D}",
    ...
    "am": {
        ...
        "tos_text": (
            f"{LOGO}\n{D}\n\n"
            "📋  <b>የአጠቃቀም ውሎች</b>\n\n"
            "ውሎቹን ያንብቡ። <b>አረጋግጣለሁ</b> ሲጫኑ ተስማምተዋል።\n\n"
            f"{D}"
        ),
        "tos_read":      "📄  ውሎቹን ያንብቡ",
        "tos_continue":  "✅  አረጋግጣለሁ",
        "tos_exit":      "❌  ውጣ",
        "exit_message":  "👋  ወጥተዋル። /start ይላኩ።",
        "maintenance":   f"{LOGO}\n{D}\n\n🔧  <b>ጥገና ላይ ነው</b>\n\nቦቱ ጊዜያዊ ጥገና ላይ ነው።\n\n{D}",
        "banned":        f"{LOGO}\n{D}\n\n🚫  <b>ተከልክለዋል።</b>\n\nስህተት ከሆነ ድጋፍን ያነጋግሩ።\n\n{D}",
        "btn_tasks":     "📋  ተግባራት",
        "btn_rewards":   "🎁  ሽልማቶች",
        ...
        "btn_back":      "🔙  ተመለስ",
        "btn_daily":     "🎁  የዕለቱ ሽልማት",
        "daily_already": "⏳  ዛሬ ወስደዋAL። ነገ ተመለሱ!",
        "task_already":  "⚠️  ቀደም ጨርሰዋታAL።",
    },
}
...
async def level_up_notify(bot, uid: int, new_level: int):
    try:
        await bot.send_message(uid,
            f"🎊  <b>LEVEL UP!</b>\n\n"
            f"You reached {lv_badge(new_level)}  <b>Level {new_level} — {lv_name(new_level)}</b>!\n\n"
            f"Keep going — bigger rewards await! 🚀", parse_mode=ParseMode.HTML)
...
async def build_profile_card(uid: int, bot, show_menu=True):
    user = await get_user(uid)
    if not user: return "", None
    ...
    card = (
        f"{LOGO}\n{D}\n\n"
        f"👤  <b>{uname}</b>\n"
        f"{lv_badge(level)}  <b>Level {level}</b>  <i>({lv_name(level)})</i>   •   🏆  Rank <b>#{rank}</b>\n"
        f"{D2}\n"
        f"💰  <b>{pts:,}</b> points\n{bar}\n"
        f"<i>{to_next:,} pts to Level {level+1}</i>\n"
        f"{D2}\n"
        f"🤝  Referrals: <b>{refs}</b>"
        + (f"   •   {sl}" if sl else "") + f"\n{D}\n"
        + ("\n🏠  <b>Main Menu</b>" if show_menu else "")
    )
    photo = None
    try:
...
async def send_main_menu(chat_id: int, lang: str, bot, old_message=None):
    ...
    card, photo = await build_profile_card(chat_id, bot, show_menu=True)
    ...
    if got_ref and referred_by:
        try:
            await ctx.bot.send_message(referred_by,
                ...
                f"You earned  <b>+{REFERRAL_POINTS} pts</b> 🏆", parse_mode=ParseMode.HTML)
        except Exception: pass
...
async def cb_daily(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if result["ok"]:
        alert = f"✅  +{result['pts']} pts!\n🔥  {result['streak']}-day streak  (+{result['streak_bonus']} bonus)"
        if result.get("leveled_up"):
            alert += f"\n\n🎉  LEVEL UP! → Level {result['new_level']} {lv_badge(result['new_level'])}"
...
def ap_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📋  Tasks",       callback_data="ap_tasks"),
         InlineKeyboardButton("🎁  Rewards",     callback_data="ap_rewards")],
        [InlineKeyboardButton("📊  Statistics",  callback_data="ap_stats"),
         InlineKeyboardButton("🎰  Lottery",     callback_data="ap_lottery")],
...
def ap_guessgame_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚽  New Score Contest",  callback_data="ap_act_newscore"),
         InlineKeyboardButton("🎯  New Result Contest", callback_data="ap_act_newresult")],
        [InlineKeyboardButton("📋  Active Contests",   callback_data="ap_act_listcontests")],
        [InlineKeyboardButton("✅  Resolve Contest",   callback_data="ap_act_resolve"),
...
async def cb_ap_guessgame(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{LOGO}\n{D}\n\n"
        f"🎯  <b>Guess Game</b>\n\n"
        f"Run score predictions or Win/Draw/Win2 polls.\n\n"
...
async def cb_guess_result(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await q.answer("⚠️  Invalid data.", show_alert=True); return
    contest = await get_contest(cid)
    if not contest or contest["status"] != "open":
        await q.answer("⏰  This contest is closed.", show_alert=True); return
    now = datetime.now(timezone.utc)
    ...
    if existing:
        ...
        await q.answer(f"✅  Already submitted: {labels.get(existing['guess'], existing['guess'])}",
                       show_alert=True); return
    await submit_guess(cid, uid, choice)
    labels = {"win1": "1️⃣ Win 1", "draw": "🤝 Draw", "win2": "2️⃣ Win 2"}
    await q.answer(f"✅  Guess submitted: {labels.get(choice, choice)}", show_alert=True)
...
async def cb_ap_share_top10(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    progress = await q.message.reply_text(
        f"📣  Broadcasting to <b>{len(all_users):,}</b> users…",
        parse_mode=ParseMode.HTML)
    for u in all_users:
        try:
...
async def handle_admin_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "draw":
        ...
        await msg.edit_text(
            ...
            f"🎯  From pool of  <b>{len(eligible)}</b> eligible users\n"
        ...
        try:
            await bot.send_message(winner["id"],
                ...
                f"You've been selected as the winner!\n"
